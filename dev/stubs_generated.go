...
// StubFile is one file a scaffold writes: its path inside the project, its
// contents, and whether the host must be able to exec it.
...
// StubLanguage is one scaffoldable language.
type StubLanguage struct {
	ID      string
	Flag    string
	Display string
	// Entry is the executable the host launches, placeholder-substituted like
	// any other rendered path.
	Entry string
	// Build holds the commands that turn the sources into Entry — empty for
	// interpreted languages, which have nothing to build.
	Build           []string
...
// StubPlaceholder is the token replaced by the project name in every file's
// CONTENTS and in its destination path and entry.
const StubPlaceholder = `__CARTRIDGE_NAME__`

const StubContractVersion = 1

// StubLanguages holds every language `capdag new` can scaffold, in contract order.
var StubLanguages = []StubLanguage{
	{
		ID:              `python`,
		Flag:            `--python`,
		Display:         `Python`,
...
`,
	...
	},
	{
		...
		Build:           []string{`go mod tidy`, `go build -buildvcs=false -o __CARTRIDGE_NAME__ .`},
...
`,
	...
	},
	{
		...
		Build:           []string{`swift build -c release`},
