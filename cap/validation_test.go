...
func Test580_rule3_different_stdin_urns(t *testing.T) {
	cap := makeTestCapWithUrnAndArgs(t,
		`cap:in="media:enc=utf-8;ext=txt";test-cap;out="media:void";type=test`,
		[]CapArg{
			NewCapArg(standard.MediaString, true, []ArgSource{stdinSource("media:enc=utf-8;ext=txt")}),
...
func Test581_rule3_same_stdin_urns_ok(t *testing.T) {
	cap := makeTestCapWithUrnAndArgs(t,
		...
		[]CapArg{
			NewCapArg(standard.MediaString, true, []ArgSource{stdinSource("media:enc=utf-8;ext=txt")}),
			NewCapArg(standard.MediaInteger, true, []ArgSource{stdinSource("media:enc=utf-8;ext=txt")}),
		},
	)
	err := ValidateCapArgs(cap)
...
func Test589_all_rules_pass(t *testing.T) {
	cap := makeTestCapWithUrnAndArgs(t,
		...
		[]CapArg{
			NewCapArg(standard.MediaString, true, []ArgSource{
				positionSource(0),
				stdinSource("media:enc=utf-8;ext=txt"),
			}),
...
func Test1294_rule11_void_input_with_stdin_rejected(t *testing.T) {
	cap := makeTestCapWithUrnAndArgs(t,
		`cap:in="media:void";test;out="media:fmt=json;record"`,
		[]CapArg{
			NewCapArg(standard.MediaString, true, []ArgSource{stdinSource("media:enc=utf-8;ext=txt")}),
		},
	)
