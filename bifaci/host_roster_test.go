...
func capGroupsFromUrns(t *testing.T, urns []string) []CapGroup {
	...
	for _, u := range urns {
		...
		caps = append(caps, *cap.NewCap(parsed, "test", "test"))
	}
...
// reconnecting, and a subsequent empty sync removes it. This is the macOS-XPC
// syncDiscoveryOutcomes parity path the daemon uses after a registry verdict
// flips a held cartridge to Listed.
func Test1879_sync_roster_adds_and_removes_registered_dir_live(t *testing.T) {
